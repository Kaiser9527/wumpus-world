...
class Agent:
    ...
    def Process (self, lastAction, percept):
        actionList2 = []
        ...
        if (self.actionList == []):
            if (percept.glitter):
                ...
                self.actionList.extend(actionList2)
            elif(percept.stench and lastAction == Action.GOFORWARD):
                if (orientation == Orientation.RIGHT):
                    self.agentLocation = [X-1,Y+1]
                    ...
                    if(location not in self.unsafeLocations):
                        ...
                        if (percept.stench):
                            ...
                            if location not in self.safeLocations:
                                ...
                                self.searchEngine.AddSafeLocation(location[0], location[1])
                                self.agentLocation = [X-1,Y]
                    if(self.w_on_l == False and self.w_on_r == False):
                        self.actionList.append(Action.TURNLEFT)
                        self.actionList.append(Action.GOFORWARD)
                        self.actionList.append(Action.TURNRIGHT)
                        self.actionList.append(Action.GOFORWARD)
